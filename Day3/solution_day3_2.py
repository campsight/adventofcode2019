...
def nb_steps(n, wireDistances, point, intersection):
    xdiff = abs(intersection[0] - point[0])
    ydiff = abs(intersection[1] - point[1])
    # print("nb_steps", n, wireDistances, point, intersection, sum(wireDistances[0:n]), xdiff, ydiff)
    return (sum(wireDistances[0:n]) + xdiff + ydiff)


closest = (0, 0)
lowest_steps = sum(wire1Distances) + sum(wire2Distances)
...
for i in range(len(wire1_coord[0]) - 1):
    for j in range(len(wire2_coord[0]) - 1):
        ...
        if (intersection != (0, 0)):  # if they intersect
            steps_to_intersection = nb_steps(i, wire1Distances, p1, intersection) + nb_steps(j, wire2Distances, q1,
                                                                                             intersection)
            # print(intersection, steps_to_intersection)
            if ((closest == (0, 0)) or (steps_to_intersection < lowest_steps)):  # first intersection always ok
                closest = intersection
                lowest_steps = steps_to_intersection

# print(line_intersection([[0,0],[8,0]],[[5,7],[5,3]]))
print(closest, " nb_steps: ", lowest_steps)